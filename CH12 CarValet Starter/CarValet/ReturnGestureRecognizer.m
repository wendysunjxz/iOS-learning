...
//  CarValet
//
//  Created by Sun Wenyan on 2015-03-01.
...
-(void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if((strokePhase ==0) && ((currPoint.y-firstTap.y)>10.0) && (currPoint.y <= prevPoint.y)){
        strokePhase = 1;
    }else if ((strokePhase == 1) && ((currPoint.x - prevPoint.x) >= kRStrokeDelta)){
        strokePhase = 2;
    }
    else if ((strokePhase == 2) &&((currPoint.y - prevPoint.y)<=kRStrokeDelta) && (currPoint.x >prevPoint.x)){
        strokePhase = 3;
